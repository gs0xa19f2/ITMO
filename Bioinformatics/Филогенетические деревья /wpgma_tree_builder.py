...
def wpgma(matrix, n):
    """
    Реализует алгоритм WPGMA для построения филогенетического дерева.
    ...
    while len(node_for_code) > 1:
        ...
        new_node = Node(n_one, n_two, min_dist / 2 - n_one.overall_dist, min_dist / 2 - n_two.overall_dist, min_dist / 2, new_code)
        node_for_code[new_code] = new_node
        for other_code in list(node_for_code):
            if other_code not in [code_one, code_two, new_code]:
                new_dist = (distance_matrix[(code_one, other_code)] + distance_matrix[(code_two, other_code)]) / 2
