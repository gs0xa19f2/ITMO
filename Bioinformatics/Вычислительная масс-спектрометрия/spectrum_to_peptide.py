def theoretical_spectrum(peptide):
    ...
    Возвращает:
    list: Отсортированный список масс фрагментов.
    """
...
def find_peptide_from_spectrum(spectrum):
    """
    Находит последовательность пептида, соответствующую заданному спектру.

    Параметры:
    spectrum (str): Спектр пептида в виде строки чисел, разделенных пробелами.

    Возвращает:
    str: Последовательность пептида.
    """
    alphabet = ['A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'V', 'W', 'Y']
    masses = [71, 103, 115, 129, 147, 57, 137, 113, 128, 113, 131, 114, 97, 128, 156, 87, 101, 99, 186, 163]
...
# Ввод спектра
sample_spectrum = input("Введите спектр пептида: ")
print("Последовательность пептида:", find_peptide_from_spectrum(sample_spectrum))
